...
/*
 * private BankTransaction parseCommaFromCSV(final String line) { final String[]
 * columns = line.split(","); final LocalDate date = LocalDate.parse(columns[0],
 * DATE_PATTERN);
 * 
 * final double amount = Double.parseDouble(columns[1]);
 * 
 ...
 * }
 * 
 * private BankTransaction parseHyphenFromCSV(final String line) {
 * 
 * final String line_fix = line.replaceAll("\"", "");
 * 
 * final String dateColumns = line_fix.substring(0, 10);
 * 
 * final String column = line_fix.replaceAll(dateColumns, ""); final String key
 * = column.substring(1, 2); final String[] columns; if (key.equals("-")) {
 * columns = column.split("-(?=[A-Za-z-]|\\d+-\\d)"); } else { columns =
 * column.split("-"); }
 * 
 * final LocalDate date = LocalDate.parse(dateColumns, DATE_PATTERN);
 * 
 ...
 * 
 * final List<BankTransaction> bankTransactions = new ArrayList<>(); for (final
 * String line : lines) { bankTransactions.add(parseHyphenFromCSV(line)); }
 * return bankTransactions; }
 */
